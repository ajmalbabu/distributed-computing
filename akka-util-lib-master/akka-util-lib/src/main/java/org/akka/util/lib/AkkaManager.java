...
import org.akka.util.lib.model.AkkaConfig;
import org.akka.util.lib.model.ClusterShardsConfig;
...
public class AkkaManager {
    ...
    private Config createConfig() {
        ...
        if (akkaConfig.getAkkaConfigFileName().length() > 0) {
            LOGGER.info("Use the provided akka.util.lib.config.file.name = {}", akkaConfig.getAkkaConfigFileName());
            config = config.withFallback(ConfigFactory.load(akkaConfig.getAkkaConfigFileName()));
        }

        LOGGER.info("akka.util.lib config {}", config.root().render(ConfigRenderOptions.defaults().setJson(true)));
