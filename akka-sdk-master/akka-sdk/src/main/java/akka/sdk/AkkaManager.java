...
import akka.sdk.model.AkkaConfig;
import akka.sdk.model.ClusterShardsConfig;
...
public class AkkaManager {
    ...
    private Config createConfig() {
        ...
        if (akkaConfig.getAkkaConfigFileName().length() > 0) {
            LOGGER.info("Use the provided akka.sdk.config.file.name = {}", akkaConfig.getAkkaConfigFileName());
            config = config.withFallback(ConfigFactory.load(akkaConfig.getAkkaConfigFileName()));
        }

        LOGGER.info("akka.sdk config {}", config.root().render(ConfigRenderOptions.defaults().setJson(true)));
